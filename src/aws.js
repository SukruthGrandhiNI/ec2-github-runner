  ...
  if (config.input.ec2BaseOs === 'win-x64') {
    ...
    for (let i = 1; i <= parseInt(config.input.numberOfRunners); i++) {
      userData.push(
        ...
        'mkdir _work',
        `./config.cmd --url https://github.com/${config.githubContext.owner}/${config.githubContext.repo} --name ${config.input.ec2BaseOs}-${label}-${instance}-${i} --token ${githubRegistrationToken} --labels ${label} --runasservice --windowslogonaccount Administrator --windowslogonpassword password12345 --unattended`,        
        'cd ..',
      );
    }
